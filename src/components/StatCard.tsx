      ...
      <div className="relative flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className={cn(
              "h-10 w-10 rounded-2xl flex items-center justify-center shrink-0 transition-bounce group-hover:scale-110 group-hover:rotate-6",
              accent ? "bg-white/20 text-primary-foreground" : `${t.bg} ${t.text}`,
            )}>
              <Icon className="h-5 w-5" />
            </div>
          </div>
          <p className={cn("text-xl sm:text-2xl lg:text-3xl font-bold mt-2 leading-tight tabular-nums tracking-tight whitespace-nowrap overflow-hidden text-ellipsis", accent ? "text-primary-foreground" : "text-foreground")}>
            {value}
          </p>
